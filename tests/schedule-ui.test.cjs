...
test('legacy presentation routes normalize to the integrated canonical views', () => {
  ...
  assert.equal(
    ...
    'schedule-order.html?view=prepare&schedule=abc&v=20260811.1',
  );
  assert.equal(
    context.routeFrameSrc('presentations?view=history&id=xyz'),
    'presentations.html?view=history&id=xyz&v=20260811.1',
  );
});
...
test('schedule UI cache versions stay aligned', () => {
  assert.match(read('app.html'), /css\/style\.css\?v=20260811\.1/);
  assert.match(read('app.html'), /js\/pwa\.js\?v=20260811\.1/);
  assert.match(read('app.html'), /params\.set\('v', '20260811\.1'\)/);
  assert.match(read('app.html'), /sss-sw-refresh-20260811\.1/);
  ...
  for (const file of ['index.html', 'schedule-calendar.html', 'schedule-order.html', 'presentations.html']) {
    assert.match(read(file), /css\/style\.css\?v=20260811\.1/);
  }
  ...
  assert.match(read('sw.js'), /sss-pwa-v20260811-1/);
