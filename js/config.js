...
async function initSidebarAuth() {
  try {
    ...
    if (!adminEl) return;
    if (admin) {
      adminEl.classList.add('visible');      // CSS: .nav-admin-item.visible { display:flex !important }
    } else {
      adminEl.classList.remove('visible');   // CSS: .nav-admin-item { display:none !important }
    }
  } catch(e) {
